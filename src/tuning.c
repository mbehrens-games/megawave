...
/* notes: 12 (c to b)                   */
static int S_ym2612_tuning_table[12];
...
short int tuning_setup()
{
  ...
  {
    S_ym2612_tuning_table[0] = 
      (int) ((256 * GENESIS_1HZ_PHASE_INCREMENT) + 0.5);
  }
  else if (G_tuning_fork == TUNING_FORK_A440)
  {
    S_ym2612_tuning_table[9] = 
      (int) ((440 * GENESIS_1HZ_PHASE_INCREMENT) + 0.5);
  }
  else if (G_tuning_fork == TUNING_FORK_A432)
  {
    S_ym2612_tuning_table[9] = 
      (int) ((432 * GENESIS_1HZ_PHASE_INCREMENT) + 0.5);
  }
  else if (G_tuning_fork == TUNING_FORK_AMIGA)
  {
    S_ym2612_tuning_table[0] = 
      (int) ((261.34375f * GENESIS_1HZ_PHASE_INCREMENT) + 0.5);
  }
  else
    return 1;
      ...
      (G_tuning_fork == TUNING_FORK_AMIGA))
  {
    for (i = 1; i < 12; i++)
    {
      S_ym2612_tuning_table[i] = 
        (int) ((S_ym2612_tuning_table[0] * mult_table[i]) + 0.5);
    }
  }
  else if ( (G_tuning_fork == TUNING_FORK_A440) ||
            (G_tuning_fork == TUNING_FORK_A432))
  {
    for (i = 0; i < 12; i++)
    {
      ...
      S_ym2612_tuning_table[i] = (int) 
        ((S_ym2612_tuning_table[9] * mult_table[i] / mult_table[9]) + 0.5);
    }
  }
...
int tuning_compute_phase_increment(char note)
{
  ...
  increment = S_ym2612_tuning_table[row];
