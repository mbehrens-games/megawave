...
/* lfo period table */
static int       S_lfo_period_table[8] = {108, 77, 71, 67, 62, 44, 8, 5};

#define CHANNEL_OP_IS_INACTIVE(num)                                            \
  (c->env[num].attenuation == 1023) &&                                         \
...
#define CHANNEL_UPDATE_OP_ENVELOPE(num)                                        \
  ...
  op_env_index[num] += ((c->tremolo.level[num] * c->amd) >> 7);                \
                                                                               \
  if (op_env_index[num] > 1023)                                                \
    op_env_index[num] = 1023;                                                  \
                                                                               \
  op_env_index[num] = op_env_index[num] << 2;
...
short int channel_is_inactive(channel* c)
{
  ...
  if ((c->algorithm == 0) || (c->algorithm == 1) ||
      (c->algorithm == 2) || (c->algorithm == 3))
  {
    if (CHANNEL_OP_IS_INACTIVE(3))
      return 1;
  }
  else if (c->algorithm == 4)
  {
    if (CHANNEL_OP_IS_INACTIVE(1) &&
        CHANNEL_OP_IS_INACTIVE(3))
      return 1;
  }
  else if ((c->algorithm == 5) || (c->algorithm == 6))
  {
    if (CHANNEL_OP_IS_INACTIVE(1) &&
        CHANNEL_OP_IS_INACTIVE(2) &&
        CHANNEL_OP_IS_INACTIVE(3))
      return 1;
  }
  else if (c->algorithm == 7)
  {
...
short int channel_setup(channel* c)
{
  ...
  /* select period */
  if ((c->tremolo_speed >= 0) && (c->tremolo_speed <= 7))
    c->tremolo.period = S_lfo_period_table[c->tremolo_speed];
  else
    c->tremolo.period = 108;

  /* set up vibrato lfo */
  /* select rows */
  for (i = 0; i < CHANNEL_NUM_OPS; i++)
  {
    if ((c->pms[i] >= 0) && (c->pms[i] <= 6))
  ...
  /* select period */
  if ((c->vibrato_speed >= 0) && (c->vibrato_speed <= 7))
    c->vibrato.period = S_lfo_period_table[c->vibrato_speed] * 4;
  else
    c->vibrato.period = 108 * 4;
  ...
  /* additional right shift by 2 is incorporated into the fb_shift variable   */
  /* to get the 16 bit output back to the 14 bits of the ym2612 op output.    */
  if (c->fb_shift > 0)
    c->fb_shift += 2;
...
short int channel_key_on(channel* c)
{
  ...
  for (i = 0; i < CHANNEL_NUM_OPS; i++)
  {
    ...
    /* ym2612 detune overflow bug, probably unnecessary for our purposes */
    c->increment[i] &= 0x1FFFFFF;
...
short int channel_update(channel* c)
{
  ...
  CHANNEL_UPDATE_OP_ENVELOPE(2)
  CHANNEL_UPDATE_OP_ENVELOPE(3)
  ...
  if (c->algorithm == 0)
  {
    ...
    CHANNEL_SET_PHASE_SHIFT_FROM_ONE_OP(1)
    CHANNEL_UPDATE_OP_LEVEL(2)
    CHANNEL_SET_PHASE_SHIFT_FROM_ONE_OP(2)
    CHANNEL_UPDATE_OP_LEVEL(3)
    level = op_level[3];
  }
  else if (c->algorithm == 1)
  {
    ...
    CHANNEL_UPDATE_OP_LEVEL(1)
    CHANNEL_SET_PHASE_SHIFT_FROM_TWO_OPS(0, 1)
    CHANNEL_UPDATE_OP_LEVEL(2)
    CHANNEL_SET_PHASE_SHIFT_FROM_ONE_OP(2)
    CHANNEL_UPDATE_OP_LEVEL(3)
    level = op_level[3];
  ...
  else if (c->algorithm == 2)
  {
    ...
    CHANNEL_SET_PHASE_SHIFT_FROM_ONE_OP(1)
    CHANNEL_UPDATE_OP_LEVEL(2)
    CHANNEL_SET_PHASE_SHIFT_FROM_TWO_OPS(0, 2)
    CHANNEL_UPDATE_OP_LEVEL(3)
    level = op_level[3];
  }
  else if (c->algorithm == 3)
  {
    ...
    CHANNEL_RESET_PHASE_SHIFT()
    CHANNEL_UPDATE_OP_LEVEL(2)
    CHANNEL_SET_PHASE_SHIFT_FROM_TWO_OPS(1, 2)
    CHANNEL_UPDATE_OP_LEVEL(3)
    level = op_level[3];
  }
  else if (c->algorithm == 4)
  {
    ...
    CHANNEL_SET_PHASE_SHIFT_FROM_ONE_OP(2)
    CHANNEL_UPDATE_OP_LEVEL(3)
    level = op_level[1] + op_level[3];
  }
  else if (c->algorithm == 5)
  {
    CHANNEL_SET_PHASE_SHIFT_FROM_ONE_OP(0)
    CHANNEL_UPDATE_OP_LEVEL(1)
    CHANNEL_UPDATE_OP_LEVEL(2)
    CHANNEL_UPDATE_OP_LEVEL(3)
    level = op_level[1] + op_level[2] + op_level[3];
  }
