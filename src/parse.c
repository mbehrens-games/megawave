...
data_tree_node* parse_file_to_data_tree(char* filename)
{
  ...
  if ((t.token == TOKEN_IDENTIFIER) && (!strcmp(t.sb, "megawave")))
  {
    root->type = DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE;
    current = root;
  ...
  while (stack_top >= 0)
  {
                ...
                (parse_state == PARSE_STATE_SUBFIELD_OR_END_OF_FIELD)))
    {
      ...
      else if (!strcmp(t.sb, "op3"))
        current->type = DATA_TREE_NODE_TYPE_FIELD_OP_3;
      else if (!strcmp(t.sb, "op4"))
        current->type = DATA_TREE_NODE_TYPE_FIELD_OP_4;
      ...
      else if (!strcmp(t.sb, "vibrato_speed"))
        current->type = DATA_TREE_NODE_TYPE_FIELD_VIBRATO_SPEED;
      else if (!strcmp(t.sb, "tremolo_speed"))
        current->type = DATA_TREE_NODE_TYPE_FIELD_TREMOLO_SPEED;
      /* envelope fields */
...
{
  if ((current_type == DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE) &&
      (parent_type != DATA_TREE_NODE_TYPE_NONE))
    return 1;
  /* channel fields */
  else if ( (current_type == DATA_TREE_NODE_TYPE_FIELD_OP_1) &&
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE))
    return 1;
  else if ( (current_type == DATA_TREE_NODE_TYPE_FIELD_OP_2) &&
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE))
    return 1;
  else if ( (current_type == DATA_TREE_NODE_TYPE_FIELD_OP_3) &&
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE))
    return 1;
  else if ( (current_type == DATA_TREE_NODE_TYPE_FIELD_OP_4) &&
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE))
    return 1;
            ...
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_OP_3)      &&
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_OP_4))
    return 1;
    ...
    return 1;
            ...
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE))
    return 1;
  else if ( (current_type == DATA_TREE_NODE_TYPE_FIELD_VIBRATO_SPEED) &&
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE))
    return 1;
  else if ( (current_type == DATA_TREE_NODE_TYPE_FIELD_TREMOLO_SPEED) &&
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE))
    return 1;
  /* envelope fields */
  ...
  /* values */
            ...
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_VIBRATO_SPEED)        &&
            (parent_type != DATA_TREE_NODE_TYPE_FIELD_TREMOLO_SPEED)        &&
...
short int parse_data_tree_load_integer(int val, int parent_type, int grand_type)
{
  ...
  if (parent_type == DATA_TREE_NODE_TYPE_FIELD_MULTIPLE)
  {
    ...
    if (grand_type == DATA_TREE_NODE_TYPE_FIELD_OP_1)
      ...
      op_num = 1;
    else if (grand_type == DATA_TREE_NODE_TYPE_FIELD_OP_3)
      op_num = 2;
    else if (grand_type == DATA_TREE_NODE_TYPE_FIELD_OP_4)
      op_num = 3;
    else
      op_num = 0;
  ...
  /* vibrato speed */
  else if (parent_type == DATA_TREE_NODE_TYPE_FIELD_VIBRATO_SPEED)
  {
    if ((val >= 0) && (val <= 7))
      G_channel.vibrato_speed = val;
    else
    {
      printf("Invalid Vibrato Speed specified. Defaulting to 0.\n");
      G_channel.vibrato_speed = 0;
    }
  }
  /* tremolo speed */
  else if (parent_type == DATA_TREE_NODE_TYPE_FIELD_TREMOLO_SPEED)
  {
    if ((val >= 0) && (val <= 7))
      G_channel.tremolo_speed = val;
    else
    {
      printf("Invalid Tremolo Speed specified. Defaulting to 0.\n");
      G_channel.tremolo_speed = 0;
    }
  }
  /* ar */
...
short int parse_data_tree_load_string(char* name, int parent_type)
{
  ...
  /* note */
  if (parent_type == DATA_TREE_NODE_TYPE_FIELD_NOTE)
  {
...
short int parse_data_tree_to_globals(data_tree_node* root)
{
  ...
  /* verify root node */
  if ((root->type != DATA_TREE_NODE_TYPE_FIELD_MEGAWAVE)  ||
      (root->sibling != NULL)                             ||
      (root->child == NULL))
  {
  ...
  while(stack_top >= 0)
  {
    ...
    {
      parse_data_tree_load_string(current->value, parent_type);
    }
